  ...
  useEffect(() => {
    ...
    const toggleGroupCollapse = (group, collapse) => {
      ...
      if (collapse) {
        ...
        if (shape) {
          // Save the current expanded size to data
          diagram.model.setDataProperty(group.data, "expandedSize", go.Size.stringify(shape.desiredSize));
        ...
        const shape = group.findObject("SHAPE");
        if (shape) {
          // Restore from saved expanded size
          if (group.data.expandedSize) {
            shape.desiredSize = go.Size.parse(group.data.expandedSize);
          } else {
